...
function Get-Workloads {
    ...
    $candidates = @(Get-CimInstance Win32_Process | Where-Object {
        $knownService = $_.Name -eq 'PresentMonService.exe' -and (
            ($_.ProcessId -eq 6192 -and $_.CreationDate.ToUniversalTime().ToString('o') -eq '2026-09-14T12:18:37.1803250Z') -or
            ($_.ProcessId -eq 9248 -and $_.CreationDate.ToUniversalTime().ToString('o') -eq '2026-09-14T12:18:38.0572420Z'))
        -not $knownService -and $_.Name -match '^(Unity|UnityShaderCompiler|bee_backend|il2cpp|MSBuild|dotnet|VBCSCompiler|csc|cl|link|lld-link|clang.*|PresentMon.*|ffmpeg|obs64|BoatAttack.*|UrpExternal.*|Megacity.*|Forest.*|SUMMIT.*|GISTutorial.*|FourSceneMain.*|DataLayout.*|ShaderHitch.*)(\.exe)?$'
    })
...
try {
    ...
    if ($record.mutexAbandoned) { throw 'Abandoned mutex requires coordinator inspection.' }
    $idleBefore = @()
