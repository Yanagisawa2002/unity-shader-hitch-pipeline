...
function Get-Workloads {
    ...
    $candidates = @(Get-CimInstance Win32_Process | Where-Object {
        $_.Name -match '^(Unity|UnityShaderCompiler|bee_backend|il2cpp|MSBuild|dotnet|VBCSCompiler|csc|cl|link|lld-link|clang.*|PresentMon.*|ffmpeg|obs64|BoatAttack.*|UrpExternal.*|Megacity.*|Forest.*|SUMMIT.*|GISTutorial.*|FourSceneMain.*|DataLayout.*|ShaderHitch.*)(\.exe)?$'
    })
