...
def freeze(attempt, player_stage, pilot_suffix, output):
    ...
    for key in ('unityVersion','buildGuid','graphicsDeviceType','graphicsDeviceName','driverIdentity','driverVersion','qualityLevelName','renderingThreadingMode','processorCount','identity'):
...
def summarize(root):
    ...
    for run in protocol['runs']:
        ...
        samples.append(dict(index=run['index'], policy=run['policy'], accepted=d['accepted'], validationSha256=sha(path),
            elapsedSeconds=d['observerElapsedSeconds'], firstPass=first, firstStaticPass=static, allUpdates=d['allUpdateIntervals'],
            peakAllocatedBytes=d['peakAllocatedBytes'], peakReservedBytes=d['peakReservedBytes'], failures=d['failures'],
    ...
    save(root/'comparison-summary.json',result)
...
if __name__ == '__main__':
    ...
    else: summarize(args.path.resolve())
